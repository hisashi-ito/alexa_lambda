...
var Adapter = /** @class */ (function (_super) {
    ...
    Adapter.prototype.registerHandlers = function () {
        ...
        for (var _a = 0, v1Handlers_1 = v1Handlers; _a < v1Handlers_1.length; _a++) {
            ...
            if (!isObject(handler)) {
                throw errorUtils_1.createAskSdkError(this.constructor.name, "Argument #" + handler.constructor.name + " was not an Object");
            }
            ...
            for (var _b = 0, eventNames_1 = eventNames; _b < eventNames_1.length; _b++) {
                ...
                if (typeof (handler[eventName]) !== 'function') {
                    throw errorUtils_1.createAskSdkError(this.constructor.name, "Event handler for '" + eventName + "' was not a function");
                }
                var handlerContext = {
    ...
    Adapter.prototype.registerV2Handlers = function () {
        ...
        this.v2RequestHandlers = this.v2RequestHandlers.concat(requestHandlers);
    };
    Adapter.prototype.execute = function () {
        ...
        if (this.resources) {
            this.i18n.use(sprintf).init({
                ...
                if (err) {
                    throw errorUtils_1.createAskSdkError(_this.constructor.name, 'Error initializing i19next: ' + err);
                }
...
function ValidateRequest() {
    ...
    try {
        if (this.appId && (requestAppId !== this.appId)) {
            ...
            var error = errorUtils_1.createAskSdkError('In validating request', 'Invalid ApplicationId: ' + this.appId);
        ...
        if (this.dynamoDBTableName && (!this._event.session.sessionId || this._event.session.new)) {
            ...
            dynamoDbPersistenceAdapter.getAttributes(this._event)
                ...
                var err = errorUtils_1.createAskSdkError(_this.constructor.name, 'Error fetching user state: ' + error);
...
function EmitEvent() {
    ...
    var skillBuilder = ask_sdk_1.SkillBuilders.custom();
    skillBuilder.addRequestHandlers.apply(skillBuilder, [new handler_1.Handler(this)].concat(this.v2RequestHandlers)).addRequestInterceptors(new copySessionAttributesInterceptor_1.CopySessionAttributesInterceptor())
...
function EmitWithState() {
    if (arguments.length === 0) {
        throw errorUtils_1.createAskSdkError(this.constructor.name, 'EmitWithState called without arguments');
    }
    ...
    if (this.listenerCount(arguments[0]) < 1) {
        throw errorUtils_1.createAskSdkError(this.constructor.name, "No 'Unhandled' function defined for event: " + arguments[0]);
    }
